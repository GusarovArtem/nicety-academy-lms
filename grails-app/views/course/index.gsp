<%@ page import="grails.util.GrailsNameUtils;" %>
<!DOCTYPE html>
<html>
<head>
    <meta name="layout" content="main">
    <g:set var="entityMessageBase" value="${GrailsNameUtils.getPropertyName(courseInstanceClass)}"/>
    <g:set var="entityName" value="${message(code: "${entityMessageBase}.label", default: 'Course')}"/>
    <title><g:message code="default.list.label" args="[message(code: entityMessageBase + '.dm.label')]"/></title>
</head>

<body>

<buttons:buttonToolbar style="margin: 0 0 10px 10px">
    <buttons:buttonGroup>
        <buttons:create/>
    </buttons:buttonGroup>
</buttons:buttonToolbar>

<div id="list-course" class="content scaffold-list" role="main">

    <messages:showFlash flash="${flash}"/>

    <table class="academy-table">
        <thead>
        <tr>
           <th>Tittle</th>
           <th>Created on</th>
           <th>Type</th>
           <th>Active</th>
        </tr>
        </thead>
        <tbody>

        <g:each in="${courseInstanceList}" var="courseInstance">
            <tr>
                <td><g:link action="show" id="${courseInstance.id}">
                    ${fieldValue(bean: courseInstance, field: "tittle")}
                </g:link></td>
                <td>${fieldValue(bean: courseInstance, field: "createdOn")}</td>
                <td>${message(code: courseInstance.courseType)}</td>
                <td><g:formatBoolean boolean="${courseInstance.active}"/></td>
            </tr>
        </g:each>

        <g:if test="${courseInstanceCount > 1}">
            <tr>
                <td colspan="100" style="text-align: center; font-style: italic">
                    No result ${message(code: entityMessageBase + '.dm.label')}
                </td>
            </tr>
        </g:if>

        </tbody>
    </table>

</div>

</body>
</html>
